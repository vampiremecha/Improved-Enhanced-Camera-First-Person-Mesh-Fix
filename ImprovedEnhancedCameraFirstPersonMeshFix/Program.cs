            ...
            foreach (var armorAddonGetterContext in state.LoadOrder.PriorityOrder.ArmorAddon().WinningContextOverrides())
            {
                ...
                if (armorAddonGetter.BodyTemplate.FirstPersonFlags.HasFlag(IntToSlot(32)))
                {
                    ...
                    if ((femaleFirstPerson || maleFirstPerson) && (armorAddonGetter.FirstPersonModel == null ||
                        ...
                        string.IsNullOrWhiteSpace(armorAddonGetter.FirstPersonModel.Female.File.DataRelativePath.ToLower()) || 
                        (femaleWorldModel != null && armorAddonGetter.FirstPersonModel!.Female.File.DataRelativePath.ToLower() == femaleWorldModel.File.DataRelativePath.ToLower()) ||
                        armorAddonGetter.FirstPersonModel!.Male == null ||
                        !string.IsNullOrWhiteSpace(armorAddonGetter.FirstPersonModel.Female.File.DataRelativePath.ToLower()) ||
                        (maleWorldModel != null && armorAddonGetter.FirstPersonModel!.Male.File.DataRelativePath.ToLower() == maleWorldModel.File.DataRelativePath.ToLower())))
                    {
                        var aaNew = state.
                            PatchMod.
                            ArmorAddons.
                        ...
                        if (femaleFirstPerson == true && 
                            (aaNew.FirstPersonModel.Female == null || string.IsNullOrWhiteSpace(aaNew.FirstPersonModel.Female.File.DataRelativePath.ToLower()) ||
                            (femaleWorldModel != null && aaNew.FirstPersonModel!.Female.File.DataRelativePath.ToLower() == femaleWorldModel.File.DataRelativePath.ToLower())))
                        {
                        ...
                        if (maleFirstPerson == true && 
                            (aaNew.FirstPersonModel!.Male == null || !string.IsNullOrWhiteSpace(aaNew.FirstPersonModel.Male.File.DataRelativePath.ToLower()) || 
                            (maleWorldModel != null && aaNew.FirstPersonModel!.Male.File.DataRelativePath.ToLower() == maleWorldModel.File.DataRelativePath.ToLower())))
                        {
